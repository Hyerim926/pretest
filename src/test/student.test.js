...
describe('student', () => {
    ...
    test.skip('중복 이메일로 회원가입 API', async () => {
        const response = await request(app).post('/v1/student/signup')
            .send({
                nickname: 'hey',
                email: 'hey@gmail.com',
    ...
    test.skip('강의등록 API | 성공 200', async () => {
    ...
    test.skip('강의등록 API | 가입되지 않은 아이디일 때', async () => {
        const response = await request(app).post('/v1/student/enrol')
            .send({
                lecture: [2],
                studentId: 100,
    ...
    test('강의등록 API | 수강신청이 가능한 강의가 하나도 없을 때', async () => {
        const response = await request(app).post('/v1/student/enrol')
            .send({
                lecture: [3, 4],
                studentId: 1,
